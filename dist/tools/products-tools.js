...
${params.includedInStore !== undefined ? `• **Store Status:** ${params.includedInStore ? 'Included only' : 'Excluded only'}` : ''}`
    ...
    async executeProductsTool(toolName, params) {
        switch (toolName) {
    ...
    async getProduct(params) {
        try {
            const response = await this.apiClient.getProduct(params.productId, params.locationId || this.apiClient.getConfig().locationId);
            if (!response.data) {
                throw new Error('No data returned from API');
            }
            return {
...
🔄 **Product has been successfully updated with the new information!**`
    ...
    async deleteProduct(params) {
        try {
            const response = await this.apiClient.deleteProduct(params.productId, params.locationId || this.apiClient.getConfig().locationId);
            if (!response.data) {
                throw new Error('No data returned from API');
            }
            return {
...
${collections.length === 0 ? '📭 **No collections found**' : collections.map((collection, index) => `
...
exports.ProductsTools = ProductsTools;
